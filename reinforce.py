...
import metrics
from utils.dataset import BatchedDataset, DataCollector, DatasetForSeq2Seq
from utils.logger import AvgStat, Logger


def shift_left(tensor, fill_token_id):
    assert len(tensor.shape) == 2
    start_ids = torch.ones((tensor.shape[0], 1)).to(tensor) * fill_token_id
...
def reward_fn(reward_model, input_ids, output_ids, args, pad_token_id=None):
    ...
    if args.softmax:
        next_state_value = torch.sum(torch.softmax(logits, dim=-1) * next_logits, dim=-1)
    else:
    ...
    with torch.no_grad(), amp_context:
        while True:
            ...
            for _ in range(epoch_length):
                ...
                model_kwargs = dict()
                model_kwargs = model._prepare_encoder_decoder_kwargs_for_generation(enc_input, model_kwargs)
                # set input_ids as decoder_input_ids
                if "decoder_input_ids" in model_kwargs:
                    input_ids = model_kwargs.pop("decoder_input_ids")
                else:
                    input_ids = model._prepare_decoder_input_ids_for_generation(enc_input.shape[0])
                ...
                for cursor in range(max_length):
                    ...
                    if args.topk is not None:
                        logits = top_k_mask_out(logits, args.topk, flat=(cursor == 0) and args.init_flat)
                ...
                ready_index = torch.arange(ready_mask.shape[0], device=ready_mask.device).masked_select(
                    ready_mask.squeeze()
                )
...
class Trainer:
    def __init__(
        ...
        self.model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[device], output_device=device)
            ...
            self.actor_procs.append(
                mp.Process(
                    ...
                    args=(
                        ...
                        self.dataset,
                        args,
                        args.world_size * num_actor,
                        self.rank * num_actor + i,
                        device,
                        free_to_go,
                    ),
                )
        ...
        dist.all_gather_object(
            epoch_lengths_per_gpu,
            sum(self.queue.get() for _ in range(num_actor)),
        )
        ...
        if os.path.exists(os.path.join(self.save_dir, "model-last/status.pt")):
            if dist.get_rank() == 0:
                logger.log(f'loading from {os.path.join(self.save_dir, "model-last")}')
    ...
    def train(self):
        ...
        while self.update_steps < self.num_training_steps:
            for in_epoch_step in range(self.epoch_length):
                ...
                ddp_context = (
                    nullcontext() if (self.iter_steps + 1) % self.iter_per_update == 0 else self.model.no_sync()
                )
                ...
                with ddp_context:
                    if enc_input.shape[0] > 0:
                        with amp_context:
                            ...
                            probs = torch.softmax(logits, dim=-1).masked_fill(
                                attention_mask.logical_not()[..., None], 0.0
                            )
                            with torch.no_grad():
                ...
                if self.iter_steps % self.iter_per_update == 0:
                    ...
                    grad_norm_avg.take(
                        torch.nn.utils.clip_grad_norm_(self.model.parameters(), self.args.max_norm).item()
                    )
                    ...
                    if dist.get_rank() == 0:
                        logs = {
                            ...
                            "train/loss": loss_avg.pop(),
                            "train/scaling": self.scaler.get_scale(),
                            "train/grad_norm": grad_norm_avg.pop(),
                            "train/entropy": entropy.item() if self.args.entropy != 0.0 else None,
                            "train/valid_rate": ready_mask.sum() / ready_mask.numel(),
                            "train/return": rewards.sum(-1).mean(),
                        }
    ...
    def save(self):
        ...
        dir_name = f"{self.save_dir}/model-{self.update_steps:07d}k"
        ...
        logger.log(f"saved at {dir_name}")
    ...
    def valid(self):
        # use greedy by default
        ofp = tempfile.NamedTemporaryFile(mode="w", encoding="utf8", delete=False)
        model = self.model.module
        if model.config.pad_token_id == self.tokenizer.vocab["<extra_id_1>"]:
            setattr(
                model.config,
                "pad_token_id",
        ...
        finished = False
        with torch.no_grad():
            input_file = os.path.join(self.args.valid_data, f"valid.{self.args.suffixes[0]}")
            reference_file = os.path.join(self.args.valid_data, f"valid.{self.args.suffixes[1]}")
            with open(input_file, encoding="utf8") as fp:
                while not finished:
                    loaded_tokens = 0
                    lines = []
                    while loaded_tokens < self.args.max_tokens:
                        line = fp.readline()
                        if line == "":
                            finished = True
                            break
                        lines.append(line.strip())
                        loaded_tokens += len(self.tokenizer(line.strip())["input_ids"])

                    inputs = self.tokenizer(lines, return_tensors="pt", padding=True)
                    input_ids = inputs["input_ids"].to(torch.device(dist.get_rank()))
                    ...
                    outputs = model.generate(
                        ...
                        length_penalty=self.args.valid_length_penalty,
                    )

                    for i in range(outputs.shape[0]):
                        ofp.write(self.tokenizer.decode(outputs[i], skip_special_tokens=True) + "\n")
                    ofp.flush()
            infered = metrics.load_data(ofp.name, self.args.lowercase_bleu)
            golden = metrics.load_ref_data(reference_file, self.args.lowercase_bleu)
            bleus = metrics.eval_bleu(golden, infered)
            ofp.close()
        ...
        if model.config.pad_token_id == model.config.decoder_start_token_id:
            setattr(
                model.config,
                "pad_token_id",
                self.tokenizer.vocab["<extra_id_1>"],
            )
        return bleus
...
if __name__ == "__main__":
    ...
    parser.add_argument("--valid-length-penalty", default=1.0, type=float)
    parser.add_argument("--lowercase-bleu", action="store_true")
    parser.add_argument("--non-parallel", action="store_true")
    ...
    if args.valid_data is None:
        setattr(args, "valid_data", args.data)
    logger.log(args)
    ...
    suffixes = (args.suffixes[0], args.suffixes[0]) if args.non_parallel else args.suffixes
    dataset_sent = DatasetForSeq2Seq(
        args.data,
        "train",
        suffixes,
        args.max_length,
    ...
    wandb.init(project="LMReward", name=args.save_dir, config=args)

    distributed_procs = []
    for i in range(args.world_size - 1):
        proc = mp.Process(target=distributed_main, args=(i, args, model, dataset, tokenizer))
        proc.start()
